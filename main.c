...
#include "DSP28x_Project.h"                         // Arquivo de cabecalho do dispositivo e exemplos incluem arquivo
                                                    ...
                                                    //
__interrupt void funcao_mediamovel(void);                 // declaracao da funcao de interrupcao
void Adc_Config(void);                              // declaracao de funcao
void freq (float);                                  //
void dutty (float);                                 //
                                                    //
float MediaMovel[100];                                 // (inteiro sem sinal de 16 bits) feito com typedef -- buffer com 100 posicoes para guardar valores do ADC
float dados[100];
float acumulador = 0;
int i = 0;
int j = 0;
...
int main(void)
{
    ...
    InitPieVectTable();                             //  trabalha junto com initpiectrl  (configura priorirdades de interrupcoes)
                                                    //
    EALLOW;                                         //
    PieVectTable.ADCINT1 = &funcao_mediamovel;                // interrupcao associada ao endereço da função adc_isr
    EDIS;                                           //
                                                    //
...
__interrupt void  funcao_mediamovel(void)                             // função usada apos a interrupcao do ADC
{
    ...
    for(i=0; i<100; i++)
    {
        ...
        for(j=aux; j < n+aux ; j++)
        {
            ...
            acumulador = acumulador + dados[n];

        }

        MediaMovel[i] = acumulador / n;
